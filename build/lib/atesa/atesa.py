...
def candidatevalues(coord_file, frame=-1, reduce=False, settings=argparse.Namespace()):
    ...
    -------
    str
        A space-separated series of CV values in the same order as they are given in candidate_op, followed
...
def initialize_eps(settings):
    ...
    if not settings.restart:  # since if restart, this file should already exist
...
def main():
        ...
        if entry[0] == 'initial_structure':
            ...
            if not literal_ops:  # only do this stuff if the OPs are not given literally
                ...
                if not isinstance(candidateops[0], list) and not isinstance(candidateops[0], str):
                    sys.exit(
                        'Error: if defining candidate_op implicitly, the first entry must be a string (including quotes)')
                if not isinstance(candidateops[0], list) and not isinstance(candidateops[1], str):
                    sys.exit(
                        'Error: if defining candidate_op implicitly, the second entry must be a string (including quotes)')
            ...
            # Obtain rc_minmax nested list from given aimless shooting output file
            # if not os.path.exists(entry[2]):
            #     sys.exit('Error: aimless shooting output file given for rc_minmax (' + str(entry[2]) + ') not found. '
            #              'Ensure that the absolute path is given.')
            # asoutlines = [[float(item) for item in line.replace('A <- ','').replace('B <- ','').replace(' \n','').replace('\n','').split(' ')] for line in open(entry[2], 'r').readlines()]
            # open(entry[2], 'r').close()
    ...
    if not rc_minmax:
        ...
        if committor_analysis or eps_settings or rc_definition:  # in this case, we need to build rc_minmax from as_out
            rc_minmax = [[],[]]
            for line in open(as_out).readlines():
                this_data = line[5:].split(' ')     # removes '[A/B] <- ' and splits into list
                while this_data[-1] == '' or this_data[-1] == '\n':
                    this_data = this_data[:-1]
                if not rc_minmax[0] and not rc_minmax[1]:
                    rc_minmax[0] = [float(x) for x in this_data]
                    rc_minmax[1] = [float(x) for x in this_data]
                else:
                    for this_index in range(len(this_data)):
                        if float(this_data[this_index]) < rc_minmax[0][this_index]:
                            rc_minmax[0][this_index] = float(this_data[this_index])
                        if float(this_data[this_index]) > rc_minmax[1][this_index]:
                            rc_minmax[1][this_index] = float(this_data[this_index])
    ...
    if resample or committor_analysis or (rc_definition and not eps_settings) or zip_for_transfer or cv_analysis:
        ...
        if resample:
            ...
            problem = 'eps_settings is defined'
        elif zip_for_transfer:
                ...
                if type(eps_dynamic_seed) == int:
                    ...
                    sys.exit('Error: eps_dynamic_seed was given as a list, but is not of the same length as the '
                             'number of EPS windows. There are ' + str((len(settings.eps_windows) - 1)) + ' EPS windows'
                                                                                                          ' but eps_dynamic_seed is of length ' + str(
                        len(settings.eps_dynamic_seed)))
                window_index = 0
                for window in range(len(settings.eps_windows) - 1):
                    settings.empty_windows.append(settings.eps_dynamic_seed[window_index])  # meaning eps_dynamic_seed[window_index] more threads need to start here before it will no longer be considered "empty"
                    window_index += 1
    ...
    if eps_settings and (resample or always_new or bootstrap_n):
        if resample:
